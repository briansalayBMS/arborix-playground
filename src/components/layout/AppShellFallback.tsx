...
export function AppShellFallback({
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="fixed top-0 bottom-0 left-[var(--nav-gutter)] z-40 flex w-[var(--nav-w)] flex-col border-r border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_92%,white)] backdrop-blur-md">
        <div className="flex min-h-0 flex-1 flex-col px-4 pb-6 pt-10">
          ...
          <div className="mono-label flex flex-col gap-3 text-[var(--color-primary)]/35">
            <span>YOU</span>
            <span>COMPANY</span>
            <span>WORK</span>
          </div>
        ...
        <div className="shrink-0 border-t-[0.5px] border-[#E2E8F0] bg-white p-4 text-left">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 pt-0.5">
              <p className="m-0 font-ui text-[14px] font-semibold leading-tight text-[var(--color-primary)]">
                {IDENTITY_ANCHOR.name}
              </p>
              <p className="m-0 mt-1 font-ui text-[10px] font-normal leading-snug text-[var(--color-secondary)]">
